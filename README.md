...
will find multiple (duplicate) answers. Doesn't matter, got results.
  ...
  * `day24part1.py` - Couldn't let these lie so here we are. Reasonably simple once I'd worked out how to represent a hexagonal grid, which is *entirely* thanks to [A handy Blog Entry](https://www.redblobgames.com/grids/hexagons/) and a little hashing it out with the Fam.
  * `day24part2.py` - Oh sure, do Conway's life on a Hex grid instead. Right. Some slight interpretations needed of when to grow the grid which I got wrong at first (grow, *then* count adjacents, *then* apply the changes). A lot quicker once I decided I should cache adjacent tile coordinates as they get looked up. Fast enough.
  * `day25part1.py` - OK, took me a couple of cracks to work out where I was going wrong calculating the loops and once I did I saw the algorithmic explosion waiting for me. I thought the solution would be a Day 13 chinese-remainder-theorem style solution, but it turns out just not being thick about (re-)calculating the number of loops was more than adequate to turn this into a couple-of-seconds solution.
  * There is no Day 25 Part 2 because that had no code to write.
